...
StructuredBuffer<float4x4> worldTransforms :register(t10);
[End_ResourceLayout]
...
VSOutputPbr VertexFunction(VSInputPbr input)
{
	...
	output.ViewId = vid;
	const int idx=input.InstId/EyeCount;
    const float4x4 wt= worldTransforms[idx];
#else
	float4x4 viewProj = ViewProj;
	const float4x4 wt= worldTransforms[input.InstId];
#endif
    
	//const float4 transformedPosWorld = mul(float4(input.Position, 1), World);
	const float4 transformedPosWorld = mul(float4(input.Position, 1), wt);
	output.PositionProj = mul(transformedPosWorld, viewProj);	
	
	output.NormalWS = mul(float4(input.Normal, 0), wt).xyz;
	#if NORMAL
	output.TangentWS = normalize(mul(float4(input.Tangent.xyz, 0), wt).xyz);
...
VSOutputPbr VertexFunction(VSInputPbr input)
{
	...
	//const float4 transformedPosWorld = mul(float4(input.Position, 1), World);
	//	uint _stride,itemCount;
	//worldTransforms.GetDimensions(itemCount, _stride);
    //float4x4 wt= worldTransforms[input.InstId % itemCount];
	//const float4 transformedPosWorld = mul(float4(input.Position, 1), World);
	const float4 transformedPosWorld = mul(float4(input.Position, 1), wt);
	output.PositionProj = mul(transformedPosWorld, viewProj);	
	...
	output.NormalWS = normalize(mul(float4(input.Normal, 0), wt).xyz);
